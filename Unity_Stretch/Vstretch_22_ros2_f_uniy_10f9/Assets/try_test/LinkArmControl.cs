...
/// </summary>
public class LinkArmControl : MonoBehaviour
{
    ...
    [Tooltip("The Transform or GameObject to move forward/backward in Unity (assign link_arm_l0)")]
    public Transform ArmLink;
    public GameObject Joint_Arm; // Alternative: GameObject reference
    ...
    public float armSpeed = 0.2f; // m/s - Increased for faster response (was 0.1, gamepad teleop equivalent ~0.2-0.3)
    ...
    private Vector3 baseLocalPosition = Vector3.zero; // Base position when arm is fully retracted
    ...
    void Start()
    {
        ...
        // Store base position (position when arm is at 0 extension)
        if (ArmLink != null)
        {
            baseLocalPosition = ArmLink.localPosition;
            // Get current position along movement axis (assume starting at 0)
            currentArmPosition = 0.0f;
        }
        else if (Joint_Arm != null)
        {
            baseLocalPosition = Joint_Arm.transform.localPosition;
            currentArmPosition = 0.0f;
        }
    ...
    void Update()
    {
        ...
        // Update Unity visualization - move along correct axis from base position
        if (ArmLink != null)
        {
            ArmLink.localPosition = CalculateNewPosition(currentArmPosition, ArmLink);
        }
        else if (Joint_Arm != null)
        {
            Joint_Arm.transform.localPosition = CalculateNewPosition(currentArmPosition, Joint_Arm.transform);
        }
    ...
    void InitializeROS2()
    {
        try
        {
            ...
            // Create publisher for arm commands (no sanitization as requested)
    ...
    /// </summary>
    Vector3 CalculateNewPosition(float extension, Transform transform)
    {
        Vector3 direction = GetMovementDirection(transform);
        return baseLocalPosition + direction * extension;
    }
